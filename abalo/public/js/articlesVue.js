...
var app = (0,vue__WEBPACK_IMPORTED_MODULE_1__.createApp)({
  ...
  watch: {
    "item_payload.searchQuery": function item_payloadSearchQuery(newVal) {
      var app = this;
  ...
  methods: {
    onSearchTimeout: function onSearchTimeout() {
      var app = this;
      if (app.views_payload.searchTimeout) clearTimeout(app.views_payload.searchTimeout);

      // Avoid too many attempts
      app.views_payload.searchTimeout = setTimeout(function () {
        app.views_payload.hasResetted = false;
        app.views_payload.hasSearched = true;
        app.searchForArticles();
      }, 500);
    },
    onCartClicked: function onCartClicked(item) {
      var app = this;
      queueCart({
        id: item.id,
        ab_name: item.ab_name,
        ab_price: item.ab_price
      });
    },
    searchForArticles: function searchForArticles() {
      var app = this;
      var search = app.item_payload.searchQuery;
      if (search) {
        app.getArticles({
          search: search
        });
    ...
    getArticles: function getArticles() {
      ...
      if (app.views_payload.searchAjax) app.views_payload.searchAjax.abort();
      var query_string = "";
      if (payload) {
